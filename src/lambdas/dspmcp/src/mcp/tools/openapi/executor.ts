...
import type { McpOpenAPIToolDefinition, SecurityScheme } from '../types.js';
...
function validateToolArguments(
  toolName: string,
  definition: McpOpenAPIToolDefinition,
...
async function applyOAuth2Security(
  ...
  if (token && securityScheme.in === 'header') {
    ...
    console.info(`Applied OAuth2 token for '${schemeName}' in header '${securityScheme.name}': ${token}`);
  ...
  if (contentType.includes('application/json') && typeof response.data === 'object' && response.data !== null) {
    if (definition.serializer) {
      try {
        ...
        console.info(`Serialized output for ${definition.name}:\n${serializedOutput}`);
